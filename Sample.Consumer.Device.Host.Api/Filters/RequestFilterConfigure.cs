...
using Sample.Consumer.Device.Infra.CrossCutting.Common.Constants;
using Sample.Consumer.Device.Infra.CrossCutting.Services;
using System;
using System.Collections.Generic;
using System.Text.RegularExpressions;

namespace Sample.Consumer.Device.Api.Filters
{
    internal class RequestFilterConfigure
    {
        internal IEnumerable<string> HeaderKeysToExtract { get; set; } = new List<string> { Headers.OriginIp, Headers.OriginDevice };
        ...
        internal void SetOriginIp(ActionExecutingContext context)
        {
            ...
            if (context.HttpContext.Request.Headers.TryGetValue("X-Forwarded-For", out StringValues header) && header.Count > 0)
                clientIp = header[0];
        ...
        internal void SetOriginDevice(ActionExecutingContext context)
        {
